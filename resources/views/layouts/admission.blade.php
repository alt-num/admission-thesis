...
<body class="min-h-screen bg-gray-50">
    <div class="flex h-screen overflow-hidden">
        ...
        <!-- Mobile sidebar -->
        <aside id="mobile-sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-gray-900 text-white transform -translate-x-full transition-transform duration-300 ease-in-out lg:hidden">
            <div class="flex flex-col h-full">
                ...
                <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                    ...
                    <div class="px-4 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">Applicants</div>
                    <a href="{{ route('admission.applicants.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg text-gray-300 hover:bg-gray-800 hover:text-white">Applicant List</a>
                    <a href="{{ route('admission.applicants.create') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg text-gray-300 hover:bg-gray-800 hover:text-white">Register Applicant</a>
                    <div class="px-4 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">Exams</div>
                    <a href="{{ route('admission.exams.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg text-gray-300 hover:bg-gray-800 hover:text-white">Exam List</a>
                    <a href="{{ route('admission.exams.create') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg text-gray-300 hover:bg-gray-800 hover:text-white">Create Exam</a>
                    <div class="px-4 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">Master Data</div>
                    <a href="{{ route('admission.courses.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg text-gray-300 hover:bg-gray-800 hover:text-white">Courses</a>
                    <a href="{{ route('admission.campuses.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg text-gray-300 hover:bg-gray-800 hover:text-white">Campuses</a>
                    <a href="{{ route('admission.departments.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg text-gray-300 hover:bg-gray-800 hover:text-white">Departments</a>
                    @php
                        $currentUser = Auth::guard('admission')->user();
                        $canViewEmployees = $currentUser && in_array($currentUser->role, ['Admin', 'Staff']);
                    @endphp
                    @if($canViewEmployees)
                        <a href="{{ route('admission.employees.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg text-gray-300 hover:bg-gray-800 hover:text-white">Employees</a>
                    @endif
                    <a href="{{ route('admission.settings.index') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg text-gray-300 hover:bg-gray-800 hover:text-white">Settings</a>
                </nav>
                <div class="flex-shrink-0 p-4 border-t border-gray-800">
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
