...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <div class="mt-6">
            <p class="text-sm text-gray-600 mb-1">Exam Location</p>
            <p class="text-lg font-medium text-gray-900">{{ $schedule->location ?? 'TBA' }}</p>
        </div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    @if($schedule->exam_code)
                        <p class="text-2xl font-bold text-indigo-600 font-mono">{{ $schedule->exam_code }}</p>
                        <p class="text-xs text-gray-500 mt-1">Provide this code to applicants in the exam room</p>
                    @else
                        <p class="text-sm text-gray-500 italic">No exam code generated yet</p>
                    @endif
                </div>
                <form method="POST" action="{{ route('admission.exams.schedules.generate-code', [$exam, $schedule]) }}" class="inline">
                    @csrf
                    <button type="submit" 
                        ...
                        {{ $schedule->exam_code ? 'Generate New Code' : 'Generate Code' }}
                    </button>
                </form>
            </div>
        </div>
                ...
                <div class="mt-4">
                    <label class="flex items-center">
                        <input type="checkbox" name="anti_cheat_enabled" value="1"
                               {{ ($schedule->anti_cheat_enabled ?? true) ? 'checked' : '' }}
